...
public class DescriptionInputFieldTest extends TestBase{
	...
	public void ipBundleElementCommon(String description) throws IOException, InterruptedException {
		...
		ipBundleElement.openIpBundleElementForm(); /*open the Custom Panel Name Page. */// open the Edit Discount for Campaign Form Page.
		ipBundleElement.typeToBundleNameFieldParam("");// taking input from the Discount Amount
		Thread.sleep (3000);
		ipBundleElement.typeAndClearDescriptionFieldParam("");
		ipBundleElement.typeToDescriptionFieldParam(description);//taking input from the Extra Bonus Validity in days
		ipBundleElement.clickActiveCheckboxField();
		//ipBundleElement.clickToSubmitButton();
